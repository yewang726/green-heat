...
from greenheatpy.master import master, cal_LCOH
from greenheatpy.projdirs import datadir
from greenheatpy.get_green_h2 import get_best_location

def get_CST_design(rm, sh, location, case, resdir, P_load=500.e3, savename=None, year=2020, costmodel='2020', OM_method='SL', fast=False):

    if fast: # interpolate data without running any actual model
        ...
        if abs(P_load-2.3e3)<0.1: # small scale application
            pm.H_helio=3
            pm.W_helio=3
        ...
        if OM_method=='SAM':
            OM_fixed=pm.c_om_cst_fix * P_load
        ...
        if savename!=None:
            np.savetxt(resdir+'/post/%s/summary_%s.csv'%(year, savename), summary, fmt='%s', delimiter=',')
        print('%s RM=%.1f, SH=%.1f, CF=%.4f, LCOH=%.2f'%(case, rm, sh, CF, LCOH))	

    else: # run real simulations
        LCOH, CF, C_cap=master(model_name='CST_TES_heat', location=location,  RM=rm, t_storage=sh, P_load_des=P_load, r_pv=0, casedir=resdir+'/post/%s'%(year), OM_method=OM_method)            


    return LCOH, CF

def get_CST_modular_design(rm, sh, location, case, resdir, P_load=500.e3, savename=None, year=2020, costmodel='2020', OM_method='SL'):
...
def get_TES_design(rm, sh, location, case,  resdir,P_load=500.e3,  savename=None, year=2020, costmodel='2020', F_pv=None):
    '''
...
def get_BAT_design(rm, sh, location, case, resdir,P_load=500.e3,  savename=None, year=2020, costmodel='2020', F_pv=None):
...
def get_PHES_design(rm, sh, location, case, resdir, P_load=500.e3, savename=None, year=2020, costmodel='2020', F_pv=None):
